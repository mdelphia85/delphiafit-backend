...
def create_sports_log(db: Session, user_id: int, data: SportsLogCreate) -> SportsLog:
    log = SportsLog(user_id=user_id, sport=data.sport, duration_minutes=data.duration_minutes, date=data.date)
...
def get_sports_logs(db: Session, user_id: int) -> List[SportsLog]:
    return db.query(SportsLog).filter(SportsLog.user_id == user_id).order_by(SportsLog.date.desc()).all()
