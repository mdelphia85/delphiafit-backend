...
def create_rep_log(db: Session, user_id: int, data: RepLogCreate) -> RepLog:
    log = RepLog(user_id=user_id, exercise_name=data.exercise_name, reps=data.reps, weight=data.weight)
...
def get_rep_logs(db: Session, user_id: int) -> List[RepLog]:
    return db.query(RepLog).filter(RepLog.user_id == user_id).order_by(RepLog.created_at.desc()).all()
